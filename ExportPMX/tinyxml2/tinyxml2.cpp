...
namespace tinyxml2
{
	...
	*/
	...
	XMLError XMLDocument::LoadFile(const char* filename)
	{
		...
		HMODULE ghmodule = GetModuleHandle(L"ExportPMX.dll");
		HRSRC hr = ::FindResource(ghmodule, MAKEINTRESOURCE(107), L"ExportPMDBoneSetting");
		if (nullptr == hr)
		{
			return _errorID;
		}
		int nResSize = SizeofResource(ghmodule, hr);
		HGLOBAL hG = LoadResource(ghmodule, hr);
		LPBYTE pData = (LPBYTE)LockResource(hG);
		const size_t size = nResSize;
		TIXMLASSERT(_charBuffer == nullptr);
		_charBuffer = new char[size + 1];
		for (size_t i = 0; i < size; i++)
		{
			_charBuffer[i] = pData[i];
		}
		_charBuffer[size] = 0;
		Parse();
		return _errorID;
